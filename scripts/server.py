    ...
    def do_GET(self):
        ...
        if parsed.path == '/api/proxy':
            ...
            if not target_url:
                ...
                return
            try:
                # Handle pipe options attached to URL: http://stream...|User-Agent=...
                actual_url = target_url
                custom_ua = qs.get('userAgent', [''])[0]
                custom_ref = qs.get('referer', [''])[0]

                if '|' in actual_url:
                    parts = actual_url.split('|')
                    actual_url = parts[0].strip()
                    pipe_params = urllib.parse.parse_qs(parts[1].replace('&', '&'))
                    if 'User-Agent' in pipe_params: custom_ua = pipe_params['User-Agent'][0]
                    if 'user-agent' in pipe_params: custom_ua = pipe_params['user-agent'][0]
                    if 'Referer' in pipe_params: custom_ref = pipe_params['Referer'][0]
                    if 'referer' in pipe_params: custom_ref = pipe_params['referer'][0]

                headers = {
                    'User-Agent': custom_ua or 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36 MintIPTV/1.0'
                }
                if custom_ref:
        ...
        if parsed.path == '/api/xc/action':
            ...
            for k, vals in qs.items():
                if k not in ['serverUrl', 'username', 'password', 'action']:
                    endpoint += f"&{urllib.parse.quote(k)}={urllib.parse.quote(vals[0])}"

            try:
                req = urllib.request.Request(endpoint, headers={'User-Agent': 'IPTVSmartersPro/3.1.5.1 (Linux; Android 11)'})
                with urllib.request.urlopen(req, timeout=20) as resp:
                    data = resp.read()
                    self.send_response(resp.status)
                    self.send_header('Content-Type', 'application/json')
